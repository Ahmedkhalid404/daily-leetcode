    ...
    int minTimeToReach(vector<vector<int>>& g) {
        ...
        auto dijkstra = [&](pair<int, int> start) {
            priority_queue<pair<pair<int, int>, pair<int, int>>,
                           vector<pair<pair<int, int>, pair<int, int>>>, greater<>>
                pq;
            ...
            while (!pq.empty()) {
                ...
                if (dist[node.first][node.second] != cost.first)
                    continue;
                ...
                for (auto it :
                    ...
                    if (isValid()) {
                        auto new_node = make_pair(node.first + it.first,
                                                  node.second + it.second);
                        int mx = max(cost.first, g[new_node.first][new_node.second]);
                        if (dist[new_node.first][new_node.second] == -1 or
                            mx + 1 + cost.second < dist[new_node.first][new_node.second]) {
                            pq.push({{mx + 1 + cost.second, cost.second ^ 1}, new_node});
                            dist[new_node.first][new_node.second] = mx + 1 + cost.second;
                            }
                    }
                     }
            }
